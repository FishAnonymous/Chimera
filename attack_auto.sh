...
OUTPUT_DIR="/data2/visitor/ASE25/Logs"
...
#         tmux send -t "monitor-$week-$date" "sleep 60 && bash /data2/visitor/ASE25/Chimera/exit_checker.sh $week $date" ENTER

        # run the simulation
        docker exec chimera bash -c "source /data/Chimera/.venv/bin/activate && python /data/Chimera/demo/daily_execution_auto_attack.py --attacker $attacker_id --attackid $attack_id"
